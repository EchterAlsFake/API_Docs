...
> [!NOTE]
> Tube8 API uses `eaf_base_api` under the hood. All standard features like proxies, logging, and asynchronous task management apply here as well.
...
# Client
The Client is the main object for the Tube8 API. Everything is handled through this object, and you should always use the Client class to get other objects.
...
# Logging
Every class in the Tube8 API has its own logger. You can set the log level, a custom log file and even a network IP + Port to log to. 
...
async def main():
    client = Client()
    video = await client.get_video("https://www.tube8.com/...")
    
    video.enable_logging(log_file="tube8.log", level=logging.DEBUG)
```

# The Video Object
You can fetch videos from Tube8 using their URLs. 
...
## Downloading Videos
Tube8 API uses the threaded HLS downloader from `eaf_base_api`. 
...
Tube8 offers different types of uploaders:
...
async def main():
    ...
    pornstar = await client.get_pornstar("https://www.tube8.com/pornstar/...")
    channel = await client.get_channel("https://www.tube8.com/channel/...")
    amateur = await client.get_amateur("https://www.tube8.com/amateur/...")
    
    # Example for Amateur / User
    print(amateur.name)
    print(amateur.views)
    print(amateur.subscribers_count)
...
# Searching
You can search Tube8 videos simply by using the `search` method:
...
async def main():
    ...
    async for video in client.search(query="latina", pages=2):
        print(video.title)
```
