# Porngo API Documentation

> - Name: porngo_api
> - Version: 1.4.1
> - Description: A Python API for the Porn Site porngo.com
...
# WARNING
> [!WARNING]
> This API is against the Terms of Services of `porngo.com`. Usage is at your risk.
...
$ `pip install porngo_api`

Or install directly from `GitHub`:

$ `pip install git+https://github.com/EchterAlsFake/porngo_api`

Optional extras:
- `pip install porngo_api[full]` (lxml extra)
...
|----------|---------------------|
| `quality` | `"480p"` or `"720p"`. Defaults to `"720p"`. If `720p` is not available, it uses `480p` instead. |
...
# Proxy Support
Proxy support is not implemented in porngo_api itself, but in its underlying network component: `eaf_base_api`.
...
# Exceptions
- `NotFound`: Raised when the server returns a 404 for the URL.
- `NetworkError`, `UnknownNetworkError`: Raised for network-related issues.
- `ProxyError`: Raised for proxy-related issues.
- `BotDetection`: Raised when bot protection is detected.
